            ...
            <template #header>
                <el-row>
                    ...
                    <el-col :span="4" :offset="8">
                        <span>{{ isSelf ? '我的关注' : 'TA的关注' }}</span>
                    </el-col>
                </el-row>
            </template>
            <el-empty v-if="concentrationList.length===0" description="没有数据哦" />
            <el-card style="border: none;" shadow="never" v-for="item in concentrationList" :key="item.id">
...
// 获取关注列表
const concentrationList = reactive([])
async function getConcentrationList() {
    if (JSON.parse(localStorage.getItem("userInfo")).id === userId.value) {
        isSelf.value = true
    }
    pageInfo.value.userId = userId.value
    await request.get("/concentration/get-concentration",{params: pageInfo.value}).then(res => {
        concentrationList.length = 0;
        concentrationList.push(...res.data.list)
...
async function follow(id,isFollow) {
    ...
    // 修改该用户的isFollow
    concentrationList.forEach(item => {
        if (item.id === id) {
